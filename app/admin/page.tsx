...
function MetricCard({ title, value, icon: Icon, trend, color }: { title: string, value: string | number, icon: any, trend?: string, color: string }) {
  return (
    <GlassPanel 
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", fontWeight: 600, marginBottom: 4 }}>{title}</p>
          <h3 style={{ fontSize: "1.75rem", fontWeight: 900, fontFamily: "var(--font-heading)" }}>{value}</h3>
          {trend && (
            <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 8, color: "var(--aurora-emerald)", fontSize: "0.8rem", fontWeight: 700 }}>
              <TrendingUp size={14} />
              <span>{trend}</span>
            </div>
          )}
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    return {
      ...
      platformAvg: celebrities.length > 0 
        ? (celebrities.reduce((acc, c) => acc + c.avgRating, 0) / celebrities.length).toFixed(1)
        : "0.0"
    };
  }, [celebrities, users]);
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ 
        ...
        backgroundImage: "radial-gradient(circle at 20% 20%, rgba(102, 126, 234, 0.05) 0%, transparent 40%), radial-gradient(circle at 80% 80%, rgba(118, 75, 162, 0.05) 0%, transparent 40%)"
      }}>
         <motion.div
         ...
         >
           <GlassPanel 
             ...
             radius="2xl" 
             style={{ width: "100%", maxWidth: 420, textAlign: "center", boxShadow: "0 20px 40px rgba(0,0,0,0.1)" }}
             glow
           >
             <div style={{ marginBottom: "var(--space-xl)" }}>
               <div style={{ 
                 width: 64, height: 64, borderRadius: 20, background: "var(--aurora-gradient)", 
                 display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto var(--space-lg)",
                 boxShadow: "0 10px 20px rgba(102,126,234,0.3)"
               }}>
                 <ShieldCheck size={32} color="white" />
               </div>
               <h1 style={{ fontFamily: "var(--font-heading)", fontSize: "1.75rem", fontWeight: 900, marginBottom: 8 }}>
                 Commander Center
               </h1>
             ...
             <div style={{ marginTop: "var(--space-xl)", paddingTop: "var(--space-lg)", borderTop: "1px solid rgba(0,0,0,0.05)", fontSize: "0.8rem", color: "var(--text-muted)" }}>
         ...
         {toast && (
           <motion.div 
             ...
             exit={{ opacity: 0, y: 20 }}
             style={{
               position: "fixed", bottom: 40, background: "rgba(245, 87, 108, 0.9)", 
               color: "white", padding: "12px 24px", borderRadius: 12, fontWeight: 700, backdropFilter: "blur(10px)"
             }}
           >
             {toast}
           </motion.div>
  ...
  return (
    <div style={{ 
      minHeight: "100vh", 
      background: "var(--bg-primary)",
      backgroundImage: "radial-gradient(circle at 0% 0%, rgba(102, 126, 234, 0.03) 0%, transparent 50%), radial-gradient(circle at 100% 100%, rgba(118, 75, 162, 0.03) 0%, transparent 50%)",
      ...
      {toast && (
        <motion.div 
          ...
          style={{
            ...
            background: "var(--aurora-gradient)", color: "white", padding: "14px 28px",
            borderRadius: "var(--radius-lg)", zIndex: 1000, fontWeight: 700, 
      ...
      {/* Hero Header */}
      <div style={{ padding: "var(--space-2xl) var(--space-2xl) 0", maxWidth: 1400, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-2xl)" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--aurora-indigo)", fontWeight: 800, fontSize: "0.85rem", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>
              <ShieldCheck size={16} />
              Admin Control
            </div>
            <h1 style={{ 
              fontFamily: "var(--font-heading)", fontSize: "3rem", fontWeight: 900, 
              background: "var(--aurora-gradient)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", 
              lineHeight: 1, marginBottom: 8 
            }}>
              Commander Hub
            </h1>
            <p style={{ color: "var(--text-muted)", fontSize: "1.1rem" }}>Streamline your platform operations.</p>
          </div>
          ...
          <div style={{ display: "flex", gap: 12 }}>
             <button
               ...
               style={{
                 ...
                 padding: "14px 28px", borderRadius: "16px", cursor: "pointer", fontWeight: 800,
                 boxShadow: "0 10px 30px rgba(102,126,234,0.3)", display: "flex", alignItems: "center", gap: 8,
                 transition: "all 0.2s ease"
               }}
             >
               <Plus size={20} />
        ...
        <motion.div 
          ...
          style={{ display: "flex", flexWrap: "wrap", gap: 20, marginBottom: "var(--space-2xl)" }}
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ position: "relative" }}>
                 ...
                 <input 
                   ...
                   style={{ 
                     padding: "12px 16px 12px 48px", borderRadius: 14, border: "1px solid rgba(0,0,0,0.05)", 
                     width: 300, background: "rgba(255,255,255,0.8)", outline: "none", fontSize: "0.95rem" 
                   }} 
                 />
              </div>
                ...
                <LayoutGroup id="tabs">
                  <button 
                    ...
                    style={{ 
                      ...
                      position: "relative", zIndex: 1, background: "transparent", color: activeTab === "celebrities" ? "var(--text-primary)" : "var(--text-muted)",
                      transition: "color 0.3s ease"
                    }}
                  >
                    Stars
                    {activeTab === "celebrities" && <motion.div layoutId="tab-bg" style={{ position: "absolute", inset: 0, background: "white", borderRadius: 10, zIndex: -1, boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }} />}
                  </button>
                  <button 
                    onClick={() => setActiveTab("users")}
                    style={{ 
                      padding: "8px 20px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 700,
                      position: "relative", zIndex: 1, background: "transparent", color: activeTab === "users" ? "var(--text-primary)" : "var(--text-muted)",
                      transition: "color 0.3s ease"
                    }}
                  >
                    Users
                    {activeTab === "users" && <motion.div layoutId="tab-bg" style={{ position: "absolute", inset: 0, background: "white", borderRadius: 10, zIndex: -1, boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }} />}
                  </button>
                </LayoutGroup>
              </div>
            </div>
            ...
            {activeTab === "celebrities" && (
              <div style={{ display: "flex", background: "rgba(0,0,0,0.05)", padding: 4, borderRadius: 12 }}>
                <button 
                  ...
                  style={{ 
                    padding: 8, borderRadius: 10, border: "none", cursor: "pointer", background: viewMode === "grid" ? "white" : "transparent",
                    color: viewMode === "grid" ? "var(--aurora-indigo)" : "var(--text-muted)", boxShadow: viewMode === "grid" ? "0 4px 12px rgba(0,0,0,0.05)" : "none"
                  }}
                >
                  <LayoutGrid size={20} />
                </button>
                <button 
                  ...
                  style={{ 
                    padding: 8, borderRadius: 10, border: "none", cursor: "pointer", background: viewMode === "table" ? "white" : "transparent",
                    color: viewMode === "table" ? "var(--aurora-indigo)" : "var(--text-muted)", boxShadow: viewMode === "table" ? "0 4px 12px rgba(0,0,0,0.05)" : "none"
                  }}
                >
                  <List size={20} />
                </button>
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: "-20px 0 50px rgba(0,0,0,0.1)", padding: "40px",
            ...
            >
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 24, flex: 1, overflowY: "auto", paddingRight: 4 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <label style={{ fontSize: "0.85rem", fontWeight: 800, color: "var(--text-muted)" }}>IDENTITY</label>
                  <input required placeholder="Stage Name" value={name} onChange={e => setName(e.target.value)}
                    style={{ padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.1)", background: "#fcfcfc", outline: "none" }} />
                </div>
                
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <label style={{ fontSize: "0.85rem", fontWeight: 800, color: "var(--text-muted)" }}>FIELD of FAME</label>
                  <input required placeholder="e.g. Actor, Footballer, Artist" value={category} onChange={e => setCategory(e.target.value)}
                    style={{ padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.1)", background: "#fcfcfc", outline: "none" }} />
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <label style={{ fontSize: "0.85rem", fontWeight: 800, color: "var(--text-muted)" }}>VISUAL ASSET (URL)</label>
                  <input required placeholder="https://image-source.com/star.jpg" value={photo} onChange={e => setPhoto(e.target.value)}
                    style={{ padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.1)", background: "#fcfcfc", outline: "none" }} />
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <label style={{ fontSize: "0.85rem", fontWeight: 800, color: "var(--text-muted)" }}>ORIGIN</label>
                  <input placeholder="Nationality" value={nationality} onChange={e => setNationality(e.target.value)}
                    style={{ padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.1)", background: "#fcfcfc", outline: "none" }} />
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <label style={{ fontSize: "0.85rem", fontWeight: 800, color: "var(--text-muted)" }}>BIOGRAPHY</label>
                  <textarea required placeholder="Narrative summary..." value={bio} onChange={e => setBio(e.target.value)}
                    style={{ padding: 14, borderRadius: 12, border: "1px solid rgba(0,0,0,0.1)", background: "#fcfcfc", outline: "none", minHeight: 140, resize: "none" }} />
                </div>

                <div style={{ display: "flex", gap: 16, marginTop: "auto", paddingTop: 32 }}>
                  <button type="button" onClick={closeModal} style={{ flex: 1, padding: 16, borderRadius: 14, border: "1px solid rgba(0,0,0,0.05)", background: "transparent", cursor: "pointer", fontWeight: 800 }}>Cancel</button>
                  <button type="submit" disabled={submitting} style={{ 
                    flex: 1.5, padding: 16, borderRadius: 14, border: "none", 
                    background: "var(--text-primary)", color: "white", cursor: "pointer", fontWeight: 800,
                    boxShadow: "0 10px 20px rgba(0,0,0,0.1)"
                  }}>
                    {submitting ? "Deploying..." : (editingId ? "Update Intel" : "Publish Star")}
                  </button>
                </div>
              </form>
            </motion.div>
